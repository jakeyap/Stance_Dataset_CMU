...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 13 12:26:35 2021

this is similar to the multitask training file "main_multitasking_user_features_keywords.py"
features used
user keywords 
user follower counts
tweet text
tweet are encoded individually

allows for ablation of individual features (txt, keywords, followers)
allows for use of bertweet based model or bert based model for comparison
...
def main():
    ...
    BERT_TOKENIZE = args.bert_tokenize
    ''' ===================================================='''
    ...
    test_dl = dataloader.df_2_dl_v5(test_df, 
    ...
    if DO_TRAIN:
        ...
        for train_idx, dev_idx in kfold_helper.split(full_train_df):
            ...
            train_dl = dataloader.df_2_dl_v5(train_df, 
...
def train(model, train_dl, dev_dl, logger, log_interval, epochs, loss_fn_s, loss_fn_v, optimizer, plotfile, modelfile, epochs_giveup=10, task='multi', mtt_weight=1.0):
    ...
    for epoch in range(epochs):
        ...
        for batch_id, minibatch in enumerate(train_dl):
            ...
            y_v =minibatch[12].to(gpu)  # viral_score
            #print(x7.dtype)
            #print(x8.dtype)
            #print(x9.dtype)
...
def get_args():
    ...
    parser.add_argument('--bert_tokenize',  action='store_true',    help='to use bert tokenizer to retokenize the dataframes')
    ''' ===================================================='''
