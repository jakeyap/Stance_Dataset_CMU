...
For hierarchical bertweet. major changes from tokenizers_v4
...
def tokenize_and_encode_pandas_pair(dataframe,stopindex=1e9,max_length=128, bertweet=False):    
    """
    ...
    for i in range(len(dataframe)):
        ...
        encoded_dict_head = tokenizer.__call__(text=head_txt,      # for root post
                                               padding='max_length',
                                               truncation=True,
        ...
        encodings_head.append(encoded_dict_head['input_ids'])
        token_ids_head.append(encoded_dict_head['token_type_ids'])
        att_masks_head.append(encoded_dict_head['attention_mask'])
        encodings_tail.append(encoded_dict_tail['input_ids'])
        token_ids_tail.append(encoded_dict_tail['token_type_ids'])
        att_masks_tail.append(encoded_dict_tail['attention_mask'])
    ...
    dataframe.insert(dataframe.shape[1],'token_type_ids_h', token_ids_head)
    dataframe.insert(dataframe.shape[1],'attention_mask_h', att_masks_head)
    dataframe.insert(dataframe.shape[1],'encoded_tweets_t', encodings_tail)
    dataframe.insert(dataframe.shape[1],'token_type_ids_t', token_ids_tail)
    dataframe.insert(dataframe.shape[1],'attention_mask_t', att_masks_tail)
    try:
...
if __name__ == '__main__':
    ...
    for i in range(len(head_ids)):
        ...
        try:
            json_data = tweet_dict[head_id]._json
            ...
            usernames_head.append(username)
            followers_head.append(followers_count)
        except KeyError:
    ...
    df = df.dropna()
    df = tokenize_and_encode_pandas_pair(df, max_length=128, bertweet=True)
    ...
    df = df.sample(frac=1)
    train_set = df.iloc[0:train_index].copy() # length 2761
    test_set  = df.iloc[train_index:].copy() # length 690
    
    torch.save(train_set, './data/train_set_128_individual_bertweet.bin')
    torch.save(test_set, './data/test_set_128_individual_bertweet.bin')
