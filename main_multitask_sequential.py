...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  8 15:13:44 2021
...
def main():
    ...
    W_ATTR =        args.weight_attr # unused option for this sequential test
    TASK =          args.task
    ...
    if DO_TRAIN:
        ...
        train_dl_s = dataloader.df_2_dl_v3(train_df, 
                                           ...
                                           logger=logger)
        train_dl_v = dataloader.df_2_dl_v3(train_df, 
                                           batch_size=TRNG_MB_SIZE, 
                                           randomize=True, 
                                           weighted_sample=W_SAMPLE, 
                                           weight_attr='viral',
        ...
        train(model=model, train_dl_s=train_dl_s, train_dl_v=train_dl_v, dev_dl=dev_dl, 
...
def train(model, train_dl_s, train_dl_v, dev_dl, logger, log_interval, epochs, loss_fn, optimizer, plotfile, modelfile, epochs_giveup=10, task='multi', mtt_weight=1.0):
    '''
    all the params needed are straightforward. except for plotfile, modelfile, ep
    model :         pytorch neural network model
    train_dl_s :    training dataloader for stance
    train_dl_v :    training dataloader for viral
    ...
    for epoch in range(epochs):
        ...
        for batch_id, minibatch in enumerate(train_dl_s):  # stance task
            if batch_id % log_interval == 0:
                logger.info(('\tStance\tEPOCH: %3d\tMiniBatch: %4d' % (epoch, batch_id)))
            ...
            loss_s.backward()           # backward prop
            optimizer.step()            # step the gradients once
            optimizer.zero_grad()       # clear gradients before next step
                
        for batch_id, minibatch in enumerate(train_dl_v): # viral task
            if batch_id % log_interval == 0:
                logger.info(('\tViral\tEPOCH: %3d\tMiniBatch: %4d' % (epoch, batch_id)))
    ...
    ax0.scatter(loss_horz, losses_s, label='train_loss_s')
    ax0.scatter(loss_horz, losses_v, label='train_loss_v')
