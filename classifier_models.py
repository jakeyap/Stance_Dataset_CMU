...
class mtt_Bertweet3(nn.Module):    # used in main_multitask_user_features.py
    ''' 
    For multitask. Viral and stance prediction. Includes the other meta data features
    For stance task, head and tail tweets are encoded by bertweet respectively. The outputs are passed into a transformer
    Similar to mtt_Bertweet2, except the meta data is fed in earlier thru the attention stack, instead of at the final step
    '''
    def __init__(self, num_labels, dropout, num_layers=2):
        super(mtt_Bertweet3, self).__init__()
        self.num_labels = num_labels

        self.bertweet = AutoModel.from_pretrained("vinai/bertweet-base")
        config = self.bertweet.config
        self.dropout0 = torch.nn.Dropout(dropout)
        self.classifier0 = torch.nn.Linear(config.hidden_size * 2 + 8, 4) # for stance
        self.classifier1 = torch.nn.Linear(config.hidden_size * 2 + 8, 2) # for viral
        
        # a single self attention layer
        single_tf_layer = nn.TransformerEncoderLayer(d_model=config.hidden_size * 2 + 8, 
                                                     nhead=8, 
                                                     dropout=config.hidden_dropout_prob)
        # create transformer blocks. the single layer is cloned internally
        self.transformer_block = nn.TransformerEncoder(single_tf_layer,
    ...
    def forward(self, 
        ...
        pooled_output_h = self.dropout0(output_h)       # apply dropout. shape=(n,768)
        pooled_output_t = self.dropout0(output_t)       # apply dropout. shape=(n,768)
        pooled_output   = torch.cat((pooled_output_h,   # concat pooled outputs. shape=(n,1539)
                                     pooled_output_t,
                                     metadata), 
                                    dim=1)
        
        pooled_output = pooled_output.unsqueeze(1)      # shape=(n,1,1544)
        tmp = self.transformer_block(pooled_output)     # apply transformer. shape=(n,1,1544)
        tmp = tmp.squeeze(1)                            # shape=(n,1544)
        tmp = self.dropout0(tmp)                        # apply dropout again. shape=(n,1544)
...
class mtt_Bertweet5(nn.Module):    # used in main_multitask_user_features_keywords.py
    ''' 
    For multitask. Viral and stance prediction. Includes the other meta data features and user keyword representation
    For stance task, head and tail tweets are encoded by bertweet respectively. The outputs are passed into a transformer
